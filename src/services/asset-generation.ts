...
export class TextGenerationService {
  async generate(prompt: string, parameters: Record<string, unknown> = {}): Promise<{
    content: string;
...
export class AssetGenerationService {
  ...
  private async processGenerationJob(jobId: string, request: GenerationRequest): Promise<void> {
    ...
    try {
      ...
      switch (request.type) {
        case 'text':
          result = await this.textService.generate(request.prompt, request.parameters);
          break;
        case 'image':
          result = await this.imageService.generate(request.prompt, request.parameters);
          break;
        case 'video':
          result = await this.videoService.generate(request.prompt, request.parameters);
          break;
        case 'music':
          result = await this.musicService.generate(request.prompt, request.parameters);
          break;
      }
