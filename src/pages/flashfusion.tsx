...
import { PromptCategory, PromptSection, FlashFusionPack } from "@/types/flashfusion";
import { 
...
export default function FlashFusionPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<PromptCategory>("automation");
  ...
  // Categories
  const categories: PromptCategory[] = ["automation", "creative", "ops", "llm", "notion", "workflow"];
      ...
      <section className="flex flex-col gap-4 py-8 md:py-10">
        ...
        <div className="flex flex-col gap-6 mt-6">
          ...
          <div className="border border-default-200 rounded-lg p-4">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-xl font-bold cursor-pointer" onClick={() => toggleSection("context")}>
                {collapsedSections.has("context") ? "▶" : "▼"} Context ({presetsBySection.context.length} presets)
              </h3>
            </div>
          ...
          <div className="border border-default-200 rounded-lg p-4">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-xl font-bold cursor-pointer" onClick={() => toggleSection("task")}>
                {collapsedSections.has("task") ? "▶" : "▼"} Task ({presetsBySection.task.length} presets)
              </h3>
            </div>
          ...
          <div className="border border-default-200 rounded-lg p-4">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-xl font-bold cursor-pointer" onClick={() => toggleSection("constraints")}>
                {collapsedSections.has("constraints") ? "▶" : "▼"} Constraints ({presetsBySection.constraints.length} presets)
              </h3>
            </div>
          ...
          <div className="border border-default-200 rounded-lg p-4">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-xl font-bold cursor-pointer" onClick={() => toggleSection("tone")}>
                {collapsedSections.has("tone") ? "▶" : "▼"} Tone ({presetsBySection.tone.length} presets)
              </h3>
            </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-6 p-4 bg-default-100 rounded-lg">
          <div className="text-lg font-semibold">
            <Trans>Estimated Tokens</Trans>: <Code color="primary">{tokenCount}</Code>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button color="primary" variant="flat" onClick={handleCopy}>
              {copySuccess ? <Trans>Copied!</Trans> : <Trans>Copy Prompt</Trans>}
            </Button>
            ...
            <div className="space-y-4">
              {context && (
                <div>
                  ...
                  <p className="text-default-600 whitespace-pre-wrap">{context}</p>
                </div>
              )}
              {task && (
                <div>
              ...
              {constraints && (
                <div>
                  <h4 className="font-semibold text-default-700">Constraints:</h4>
                  <p className="text-default-600 whitespace-pre-wrap">{constraints}</p>
                </div>
              )}
              {tone && (
                <div>
